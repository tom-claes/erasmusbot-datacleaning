        ...
        for field in ['title', 'description', 'content']:
            ...
            if field == 'content':
                ...
                for i, chunk in enumerate(chunks):
                    cleaned_item = OrderedDict()  # Use an OrderedDict
                    cleaned_item['id'] = str(id_counter)  # Add the id first
                    cleaned_item['title'] = item['title']
                    cleaned_item['source'] = item['source']
                    cleaned_item['description'] = item['description']
                    cleaned_item['content'] = chunk

                    cleaned_data.append(cleaned_item)

                    # Create a valid filename from the title
                    filename = re.sub('[^0-9a-zA-Z]+', '-', item['title']).strip('-')
                    filename = f"cleaned_data/{filename}-{i+1}.json"

                    # Write the chunk to a separate JSON file
                    with open(filename, 'w', encoding='utf-8') as json_file:
                        json.dump(cleaned_item, json_file, ensure_ascii=False, indent=4)
...
def main():
    ...
    with open('cleaned_data2.json', 'w', encoding='utf-8') as json_file:
